...
echo "=== Restart MariaDB to clear locks ==="
...
cat > /etc/nginx/sites-available/jowabuzz.com << 'EOF'
server {
...
EOF
cat > /etc/nginx/sites-available/jowabuzz.shop << 'EOF'
server {
    listen 80;
...
EOF
cat > /etc/nginx/sites-available/jowabuzzaffiliate.com << 'EOF'
server {
    listen 80;
...
while not stdout.channel.exit_status_ready():
    if stdout.channel.recv_ready():
        print(stdout.channel.recv(8192).decode('utf-8', errors='replace'), end='', flush=True)
    if stderr.channel.recv_stderr_ready():
        sys.stderr.write(stderr.channel.recv_stderr(8192).decode('utf-8', errors='replace'))
    time.sleep(0.5)
