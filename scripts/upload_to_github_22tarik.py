#!/usr/bin/env python3
"""Full project upload to https://github.com/12Jubayer/22tarik.git"""
import os
import shutil
import subprocess
import sys
...
GITHUB_REPO = 'https://github.com/12Jubayer/22tarik.git'
...
def fetch_server_archive():
    ...
    archive_remote = '/tmp/jowabuzz-22tarik-upload.tar.gz'
...
def build_upload_tree(local_tar):
    work = Path(tempfile.mkdtemp(prefix='jowabuzz-22tarik-'))
    log('Extracting server archive...')
...
def prepare_and_push(upload_root: Path):
    ...
    run([
        ...
        'commit', '-m', 'Full Jowabuzz project upload (22tarik)',
    ], cwd=upload_root)
    ...
    if token:
        push_url = f'https://{token}@github.com/12Jubayer/22tarik.git'
        run(['git', 'remote', 'set-url', 'origin', push_url], cwd=upload_root)
