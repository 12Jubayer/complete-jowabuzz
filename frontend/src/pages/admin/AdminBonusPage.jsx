...
import {
  ...
  fetchAdminBonuses,
  rejectAdminBonus,
} from '../../services/adminBonusService';
...
export default function AdminBonusPage() {
  ...
  const loadBonuses = useCallback(async () => {
    setLoading(true);
    try {
      const result = await fetchAdminBonuses({
        startDate,
        endDate,
  ...
  return (
    <>
      ...
      <div className="space-y-4">
        <h2 className="text-[28px] font-bold tracking-tight text-slate-900">Bonus</h2>
                ...
                {loading ? (
                  ...
                  rows.map((row) => (
                    <tr key={row.id} className="border-t border-slate-100">
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex rounded-full border border-purple-200 bg-purple-50 px-2.5 py-0.5 text-xs font-medium text-purple-700">
                          Bonus
                        </span>
                      </td>
