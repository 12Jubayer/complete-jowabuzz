...
conn.on('ready', () => {
  ...
  // Commands to delete all files inside frontend and backend on the server
  const commands = [
  ...
  conn.exec(commands, (err, stream) => {
    ...
    stream.on('close', (code, signal) => {
      ...
      conn.end();
      if (code === 0) {
        console.log('Remote frontend and backend files deleted successfully!');
        process.exit(0);
      } else {
        console.error('Failed to delete remote files.');
        process.exit(code || 1);
      }
    }).on('data', (data) => {
